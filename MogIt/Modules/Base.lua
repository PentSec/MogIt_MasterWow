...
function mog.base.AddSlot(slot, addon)
    ...
    return function(id, display, itemname, itemLevel, quality, lvl, faction, class, bind, slot, sheath, source,
        sourceid, zone, sourceinfo)
        tinsert(list, id);
...
mog.base.Help = {L["Left click to cycle through items"], L["Right click for additional options"],
                 L["Shift-left click to link"], L["Shift-right click for item URL"],
                 L["Ctrl-left click to try on in dressing room"], L["Ctrl-right click to preview with MogIt"]}
...
function mog.base.SortColour(items)
    ...
    return {mog:GetData("display", display, "colour1"), mog:GetData("display", display, "colour2"),
            mog:GetData("display", display, "colour3")};
    -- return mog:GetData("display", display, "colours");
end
-- //

-- // Register Modules
local addons = {"MogIt_Cloth", "MogIt_Leather", "MogIt_Mail", "MogIt_Plate", "MogIt_OneHanded", "MogIt_TwoHanded",
                "MogIt_Ranged", "MogIt_Other", "MogIt_Accessories"};

-- Create a table to store the names of the registered addons
...
for _, addon in ipairs(addons) do
    ...
    if loadable then
        local module = mog:RegisterModule(addon, tonumber(GetAddOnMetadata(addon, "X-MogItModuleVersion")), {
            ...
            filters = {"name", "level", "itemLevel", "faction", "class", "source", "quality", "bind", "chestType",
                       "sheath", (addon == "MogIt_OneHanded" and "slot") or nil},
        ...
        -- Add the registered addon to the table
        registeredAddons[addon] = true
        -- Dirty fix for now - if the "slot" filter is not present the array is broken unless we do this
        tinsert(module.filters, "hasItem");
    end
end

-- Registers modules that are not in the list of base addons as “extra modules”.
for i = 1, GetNumAddOns() do
    local name, title, _, _, loadable = GetAddOnInfo(i);
    if loadable and not registeredAddons[name] then
