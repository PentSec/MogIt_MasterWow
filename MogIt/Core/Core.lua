...
function mog:RegisterModule(name, version, data)
    ...
    elseif type(version) ~= "number" or version < mog.moduleVersion then
        mog:Error(L["The \124cFFFFFFFF%s\124r module needs to be updated to work with this version of MogIt."]:format(
            name));
        return;
...
function mog:BuildList(top, module)
    if (module and mog.active and mog.active.name ~= module) then
        return
    end
            ...
            if item then
                ...
                if hasItem then
                    if (slot ~= INVSLOT_HEAD or ShowingHelm()) and (slot ~= INVSLOT_BACK or ShowingCloak()) then
                        frame:TryOn(mog.mogSlots[slot] and select(6, GetTransmogrifySlotInfo(slot)) or
                                        GetInventoryItemID("player", slot), slotName);
                    end
                else
...
function mog:AddData(data, id, key, value)
    if not data and id and key then
        return
    end
...
-- // Slot Conversion
mog.slots = {"HeadSlot", "ShoulderSlot", "BackSlot", "ChestSlot", "ShirtSlot", "TabardSlot", "WristSlot", "HandsSlot",
