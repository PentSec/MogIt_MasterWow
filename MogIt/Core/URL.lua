...
function mog:ShowURL(id, sub, url, force)
    ...
    if not itemName then
        print("Error: No se pudo obtener el nombre del ítem.")
        return;
    end
    ...
    if mog.url[url] and mog.url[url][sub] then
        ...
        if type(mog.url[url][sub]) == "function" then
            text = mog.url[url][sub](itemName);
        else
            text = mog.url[url][sub]:format(itemName);
        end
        if text then
            StaticPopup_Show("MOGIT_URL", mog.url[url].fav and "\124T" .. mog.url[url].fav .. ":18:18\124t " or "", url,
                text);
            return true;
        end
    end
...
mog:AddURL("Wowhead", {
    ...
    item = L["https://www.google.com/"] .. "search?q=%s",
    set = L["https://www.google.com/"] .. "search?q=%s",
    npc = L["https://www.google.com/"] .. "search?q=%s",
    spell = L["https://www.google.com/"] .. "search?q=%s",
