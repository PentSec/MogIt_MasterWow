...
function mog:ActivatePreview(preview)
    ...
    preview.Bg:SetTexture("Interface\\Tooltips\\UI-Tooltip-Background", true);
    preview.Bg:SetVertexColor(0.2, 0.2, 02, 0.8);
...
local function resizeOnMouseUp(self)
    ...
    if mog.db.profile.singlePreview and mog.db.profile.previewUIPanel then
        return
    end
    local f = self:GetParent();
    ...
    if slot then
        if slot == "MainHandSlot" or slot == "SecondaryHandSlot" then
            ...
            if invType == "INVTYPE_WEAPON" then
                ...
                if preview.slots["MainHandSlot"].item and not preview.slots["SecondaryHandSlot"].item and
                    not preview.data.twohand then
                    slot = "SecondaryHandSlot";
                end
            end

            if invType == "INVTYPE_2HWEAPON" or invType == "INVTYPE_RANGED" or
                (invType == "INVTYPE_RANGEDRIGHT" and subType ~= LBI["Wands"]) then
...
function mog.view.DelItem(slot, preview)
    if not (preview and slot and preview.slots[slot].item) then
        return
    end
