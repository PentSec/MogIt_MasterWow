...
-- setting bg
mog.frame.TitleText:SetText("MogIt - MasterWow");
...
MogItFrameBg:SetTexture("Interface\\Tooltips\\UI-Tooltip-Background", true);
MogItFrameBg:SetVertexColor(0.2, 0.2, 0.2, 0.8);
...
function ModelFramePrototype:ShowIndicator(name)
    if not mog.indicators[name] then
        return
    end
...
function mog.DressFromPreview(model, previewFrame)
    ...
    for id, slot in pairs(previewFrame.slots) do
        if slot.item then
            model:TryOn("item:" .. slot.item ..
                            (previewFrame.data.weaponEnchant and ":" .. previewFrame.data.weaponEnchant or ""),
                slot.slot);
        end
    end
end
